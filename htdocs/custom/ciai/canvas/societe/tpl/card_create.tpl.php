...
*/
    ...
    elseif ($action == 'edit')
    {
        ...
        if ($socid)
        {
            ...
            print '<form enctype="multipart/form-data" action="'.$_SERVER["PHP_SELF"].'?socid='.$object->id.'&canvas=societe@ciai" method="post" name="formsoc">';
    ...
    else
    {
        ...
        if($mysoc->localtax1_assuj=="1" && $mysoc->localtax2_assuj=="1")
        {
            ...
            {
            	print '<form method="post" action="'.$_SERVER['PHP_SELF'].'?socid='.$object->id.'&canvas=societe@ciai">';
...
*/
		...
		if (!empty($conf->incoterm->enabled))
		{
            ...
            if ($user->rights->societe->creer) print '<a href="'.DOL_URL_ROOT.'/societe/soc.php?socid='.$object->id.'&action=editincoterm&canvas=societe@ciai">'.img_edit().'</a>';
            else print '&nbsp;';
            print '</td></tr></table>';
            print '</td>';
            print '<td colspan="3">';
		...
		if (empty($reshook))
		{
	        ...
	        {
	        	print '<div class="inline-block divButAction"><a class="butActionDelete" href="soc.php?action=merge&canvas=societe@ciai&socid='.$object->id.'" title="'.dol_escape_htmltag($langs->trans("MergeThirdparties")).'">'.$langs->trans('Merge').'</a></div>';
	        }
