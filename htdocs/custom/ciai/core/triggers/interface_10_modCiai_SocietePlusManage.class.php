...
/**
 * 	\file		core/triggers/interface_10_modCiai_SocietePlus.class.php
 * 	\ingroup	ciai
 * 	\brief		Trigger gestione Societe Plus class
 */
...
if ($dolibarr_version[0] < 3 || ($dolibarr_version[0] == 3 && $dolibarr_version[1] < 7)) { // DOL_VERSION < 3.7
	/**
	 * Class SocietePlusManage
	 */
	abstract class SocietePlusManage
	{
	}
} else {
	/**
	 * Class SocietePlus
	 */
	abstract class SocietePlusManage extends DolibarrTriggers
	{
	}
}

/**
 * Class InterfaceSocietePlusManage
 */
class InterfaceSocietePlusManage extends SocietePlusManage
{
	/**
	...
	public function __construct($db)
	{
		...
		$this->family = "ciai";
		$this->description = "Trigger di questo modulo gestiscono gli oggetti della classe Societe Plus";
		// 'development', 'experimental', 'dolibarr' or version
		$this->version = 'development';
		$this->picto = 'ciai@ciai';
	}
	...
	public function runTrigger($action, $object, User $user, Translate $langs, Conf $conf)
	{
		...
		elseif ($action == 'COMPANY_CREATE') {
		    ...
		    $ret = $this->companyCreate($action, $object, $user, $langs, $conf);
			dol_syslog(
				"Trigger '" . $this->name . "' for action '$action' launched by " . __FILE__ . ". id=" . $object->id
			);
		} elseif ($action == 'COMPANY_MODIFY') {
		    
		    $ret = $this->companyModify($action, $object, $user, $langs, $conf);
			dol_syslog(
				"Trigger '" . $this->name . "' for action '$action' launched by " . __FILE__ . ". id=" . $object->id
			);
		} elseif ($action == 'COMPANY_DELETE') {
		    
		    $ret = $this->companyDelete($action, $object, $user, $langs, $conf);
			dol_syslog(
				"Trigger '" . $this->name . "' for action '$action' launched by " . __FILE__ . ". id=" . $object->id
			);
		}
	...
	/**
	 * Creazione nuovo soggetto terzo
	 * 
	 * @param string $action: actione della pagina di origine )
	 * @param unknown $object: Societe
	 * @param unknown $user: User
	 * @param unknown $langs: Translate
	 * @param unknown $conf: global conf
	 */
	 ...
	 * @param unknown $conf: global conf
	 */
	function companyModify($action, $object, $user, $langs, $conf) {
	    $val = 0;
	...
	/**
	 * Eliminazione nuovo soggetto terzo
	 *
	 * @param string $action: actione della pagina di origine )
	 * @param unknown $object: Societe
	 * @param unknown $user: User
	 ...
	 * @param unknown $conf: global conf
	 */
	function companyDelete($action, $object, $user, $langs, $conf) {
	    $val = 0;
