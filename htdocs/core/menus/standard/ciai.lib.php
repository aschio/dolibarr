<?php
/* Copyright (C) 2010-2014 Laurent Destailleur  <ciai@users.sourceforge.net>
...
/**
 *  \file		htdocs/core/menus/standard/ciai.lib.php
 *  \brief		Library for file ciai menus
 */
require_once DOL_DOCUMENT_ROOT.'/core/class/menubase.class.php';


/**
 * Core function to output top menu ciai
 *
 ...
 * @return	int						0
 */
function print_ciai_menu($db,$atarget,$type_user,&$tabMenu,&$menu,$noout=0)
{
	...
	$showmode=dol_ciai_showmenu($type_user, $tmpentry, $listofmodulesforexternal);
	if ($showmode)
	{
	...
	$menuArbo = new Menubase($db,'ciai');
	$newTabMenu = $menuArbo->menuTopCharger('','',$type_user,'ciai',$tabMenu);	// Return tabMenu with only top entries
	...
	for($i = 0; $i < $num; $i++)
	{
		...
		$showmode=dol_ciai_showmenu($type_user,$newTabMenu[$i],$listofmodulesforexternal);
		if ($showmode == 1)
		{
...
/**
 * Core function to output left menu ciai
 *
 * @param	DoliDB		$db                 Database handler
 ...
 */
function print_left_ciai_menu($db,$menu_array_before,$menu_array_after,&$tabMenu,&$menu,$noout=0,$forcemainmenu='',$forceleftmenu='',$moredata=null)
{
	...
	if ($mainmenu)
	{
		...
		*/
		...
		$menuArbo = new Menubase($db,'ciai');
		$newmenu = $menuArbo->menuLeftCharger($newmenu,$mainmenu,$leftmenu,(empty($user->societe_id)?0:1),'ciai',$tabMenu);
		//var_dump($newmenu->liste);    //
 ...
 */
function dol_ciai_showmenu($type_user, &$menuentry, &$listofmodulesforexternal)
{
	global $conf;
