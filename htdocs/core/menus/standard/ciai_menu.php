<?php
/* Copyright (C) 2005-2013 Laurent Destailleur  <ciai@users.sourceforge.net>
...
/**
 *	\file       htdocs/core/menus/standard/ciai_menu.php
 *	\brief      Menu ciai manager
 */


/**
 *	Class to manage menu ciai
 */
class MenuManager
{
	var $db;
    ...
    function loadMenu($forcemainmenu='',$forceleftmenu='')
    {
        ...
        else
       {
        ...
        $tabMenu=array();
        $menuArbo = new Menubase($this->db,'ciai');
        $menuArbo->menuLoad($mainmenu, $leftmenu, $this->type_user, 'ciai', $tabMenu);
    ...
    function showmenu($mode, $moredata=null)
    {
        ...
        require_once DOL_DOCUMENT_ROOT.'/core/menus/standard/ciai.lib.php';
        ...
        {
        	if ($mode == 'top')  print_ciai_menu($this->db,$this->atarget,$this->type_user,$this->tabMenu,$this->menu,0);
        	if ($mode == 'left') print_left_ciai_menu($this->db,$this->menu_array,$this->menu_array_after,$this->tabMenu,$this->menu,0,'','',$moredata);
        }
        else
		{
        	$conf->global->MAIN_SHOW_LOGO=0;
        	if ($mode == 'top')  print_left_ciai_menu($this->db,$this->menu_array,$this->menu_array_after,$this->tabMenu,$this->menu,0);
        	if ($mode == 'left') print_ciai_menu($this->db,$this->atarget,$this->type_user,$this->tabMenu,$this->menu,0);
		}

		if ($mode == 'topnb')
		{
		    print_ciai_menu($this->db,$this->atarget,$this->type_user,$this->tabMenu,$this->menu,1);
        ...
        if ($mode == 'jmobile')
        {
        	...
        	foreach($this->menu->liste as $key => $val)		// $val['url','titre','level','enabled'=0|1|2,'target','mainmenu','leftmenu'
        	{
        		...
        		if ($val['enabled'] == 1)
        		{
        			...
        			$tmpleftmenu='all';
        			$submenu=new Menu();
	        		print_left_ciai_menu($this->db,$this->menu_array,$this->menu_array_after,$this->tabMenu,$submenu,1,$tmpmainmenu,$tmpleftmenu);
